...
onMounted(() => refresh(route.params.id))
...
<template>
  <AppPage vertical :title="recipe.name">
    <template #actions>
      <n-button @click="$router.push({name: 'edit_recipe', params: {id: $route.params.id}})">upravit</n-button>
    </template>
      ...
      <n-flex>
        ...
        <n-list>
          ...
          <n-list-item>
            ...
            <n-flex size="small">
              <n-tag v-for="tag in recipe.tags" :key="tag.id" round>{{tag.name}}</n-tag>
            </n-flex>
          </n-list-item>
        </n-list>
      </n-flex>
