...
interface IEmailListModalProps {
    open: boolean
    onClose: () => void
    lists: { users: (EventApplication | User)[], title: string }[]
    title: string
}
            ...
            <div onClick={e => { e.stopPropagation() }} className={styles.content} >
                <div className={styles.modalFormBox}>
                    <div className={styles.emailList}>
                        {lists?.map((list: { users: (EventApplication | User)[], title: string }) => (
                            <div className={styles.emailList} key={list.title}>
                                ...
                                <div className={styles.emailListContent}>
                                    {list.users?.map((application: (EventApplication | User), index) => (
                                        `${index !== 0 ? ', ' : ''}${application.first_name} ${application.last_name} <${application.email}>`
                                    ))}
                                </div>
                            </div>
                        ))}
