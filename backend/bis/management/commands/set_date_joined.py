    ...
    def handle(self, *args, **options):
        ...
        for page_num in paginator.page_range:
            ...
            for user in paginator.page(page_num):
                ...
                dates.extend(
                    date(membership.year, 1, 1) for membership in user.memberships.all()
                )
                dates.extend(
                    qualification.valid_since
                    for qualification in user.qualifications.all()
                )
                dates.extend(
                    event.start for event in user.events_where_was_organizer.all()
                )
                dates.extend(
                    event.event.start for event in user.participated_in_events.all()
                )
