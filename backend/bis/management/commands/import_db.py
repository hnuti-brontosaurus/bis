...
def get_event_start(item):
    ...
    if start:
        start = datetime(
            ...
            month=start.month,
            day=start.day,
            tzinfo=ZoneInfo(key="Europe/Prague"),
        )
        if item["sraz"]:
            time = item["sraz"]
            time = time.replace(".", ":")
...
class Command(BaseCommand):
    ...
    event_category_map = {
        ...
        "13": EventCategory.objects.get(
            slug="public__educational__educational_with_stay"
        ),
    ...
    diet_category_map = {
        ...
        "1": [
            DietCategory.objects.get(slug="meat"),
            DietCategory.objects.get(slug="vege"),
        ],
    ...
    def reset_cache(self):
        self.user_map = {
            _id: u for u in User.objects.all() for _id in u._import_id.split(",") if _id
        }
        ...
        for i, (id, item) in enumerate(data["adresa"].items()):
            ...
            if street and city and zip_code:
                UserAddress.objects.update_or_create(
                    user=user,
                    defaults=dict(street=street, city=city, zip_code=zip_code),
                )
            ...
            if street and city and zip_code:
                UserContactAddress.objects.update_or_create(
                    user=user,
                    defaults=dict(street=street, city=city, zip_code=zip_code),
                )
        ...
        for i, (id, item) in enumerate(data["darce"].items()):
            ...
            donor = Donor.objects.update_or_create(
                ...
                defaults=dict(
                    ...
                    regional_center_support=self.administration_unit_map.get(
                        item["rc_podpora"]
                    ),
                    basic_section_support=self.administration_unit_map.get(
                        item["zc_podpora"]
                    ),
                ),
            )[0]
            if item["vs"]:
                VariableSymbol.objects.update_or_create(
                    variable_symbol=item["vs"], defaults=dict(donor=donor)
                )
    ...
    def import_memberships(self, data):
        for i, (id, item) in enumerate(data["clen"].items()):
            ...
            if (
                item["klub"] not in self.administration_unit_map
                or item["adresa"] not in self.user_map
            ):
                continue
    ...
    def import_events(self, data):
        ...
        for item in data["porada"]:
            if item["klub"] not in self.administration_unit_map:
                continue

            event_organizer_map.setdefault(item["akce"], []).append(
        ...
        for i, (id, item) in enumerate(data["akce"].items()):
            ...
            if alternative_registration_link:
                alternative_registration_link = (
                    "http://" + alternative_registration_link
                )
